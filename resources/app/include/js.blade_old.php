...
<script>
    ...
    function onChangeAsset(obj){
        ...
        $.ajax({
            ...
            success:function(response) {
                ...
                $.each(response, function(index, obj){
                    for (let i = 0; i < obj.length; i++) {
                        $('#noidasset').append('<option value="'+ obj[i].id +'">'+ obj[i].no_id_asset +'</option>');
                    }
                })
            },
            error:function(error){
    ...
    function onChangeteam(obj){
        ...
        $.ajax({
            method  : "POST",
            url     : "{{url('/api/realisasi-personil')}}",
            data    :{
                "_token"  : "{{ csrf_token() }}",
                "id_cabang" : team
            },
    ...
    function onCheckDateFromAssetTo(){
        ...
        // if(f_datefrom.length < 0 && f_dateto.length < 0 && f_dateto < f_datefrom){
        //     showModalAlert('Tanggal Pengadaan Harus lebih kecil dari tanggal terima.')
        //      $('#tglpengdaan').val(null).trigger('change');
        //      $('#tglterima').val(null).trigger('change');
        // }

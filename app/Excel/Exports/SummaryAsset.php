...
class SummaryAsset implements WithEvents
{
    ...
    public function __construct( $datefrom, $dateto,  $cabang,  $lokasi,  $category)
    {
        ...
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                //                     ->leftjoin('m_tipe_asset', 'm_subtipe_asset.id_tipe_asset', 'm_tipe_asset.id')->get();
                 //get tipe asset
                 $dataTipeTeam      = m_tipe_team::where('flag_Active', 1)
                                     ->get();  
                 $dataTipeJabatan   = m_tipe_Jabatan::where('flag_Active', 1)
                                            ->get();  
                  $posMergeCell = 0;                             
                  $posCell  = 3 ;
                  $posCell2 =  3;
                  ...
                  if($dataTipeTeam != null){
                              
                    $posMergeCell = count($dataTipeTeam)-1;
                    foreach($dataTipeTeam as $k_dataTipeTeam){
                       
                        // var_dump($k_dataTipeTeam->tipe_team_name);
                        $colCell = GlobalHelper::numToAlpha($posCell);
                        array_push($arrIdTipeTeam, $k_dataTipeTeam->id);
                        $event->sheet->setCellValue($colCell."2", $k_dataTipeTeam->tipe_name);
                        $posCell++;
                ...
                $event->sheet->setCellValue($colCell."1", 'Personil');

                if($dataTipeJabatan != null){
                    $posMergeCell = count($dataTipeJabatan)-1;      
                    $posCell2 =  $posCell;
                    foreach($dataTipeJabatan as $k_dataTipeJabatan){
                       
                        // var_dump($k_dataTipeJabatan->tipe_jabatan_name);
                        $colCell = GlobalHelper::numToAlpha($posCell);
                        array_push($arrIdTipeJabatan, $k_dataTipeJabatan->id);
                        $event->sheet->setCellValue($colCell."2", $k_dataTipeJabatan->tipe_name);
                        $posCell++;
                ...
                if($this->cabang) $sql .= " and aa.id = {$this->cabang}";
                ...
                if($data != null){
                    ...
                    foreach($data as $k_data) {
                        ...
                        for($i=0; $i<count($arrIdTipeTeam); $i++){
                            //get count team
                                $tmpPosTeam = $tmpPosCellDetail;
                                $sql3 = "
                                SELECT sum(his_team_asset.last_qty) as jml_data  from  his_team_asset
                                left join m_team_asset a ON a.id = his_team_asset.id_team_asset
                                
                                where his_team_asset.id_cabang = {$k_data->id} and a.id_tipe_team = {$arrIdTipeTeam[$i]} and a.flag_active = 1 
                            ";
                        ...
                        for($i=0; $i<count($arrIdTipeJabatan); $i++){
                            //get data personil
                                $tmpPosPersonil = $tmpPosCellDetail;
                                $sql4 = "
                                SELECT sum(a.last_qty) as jml_data  from his_jabatan_asset a
                                left join m_jabatan_asset b on a.id_jabatan_asset = b.id
                                where a.id_cabang = {$k_data->id} and b.id_tipe_jabatan = {$arrIdTipeJabatan[$i]} and a.flag_active = 1 
                            ";
                        ...
                        for($i=0; $i<count($arrIdSubAsset); $i++){
                            ...
                            if($this->cat_asset) $sql2 .= " and a.id_cat_asset = {$this->cat_asset}";
